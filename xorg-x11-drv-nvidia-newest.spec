...
Source5:       nvidia-newest-init
Source10:      nvidia-newest-config-display
Source11:      nvidia-newest-README.Fedora
...
Requires:        nvidia-newest-kmod >= %{version}
Requires(post):  nvidia-newest-kmod >= %{version}
...
Provides:      nvidia-newest-kmod-common = %{version}
...
Obsoletes:     nvidia-newest-kmod < %{version}
...
# Install initscript
install -D -p -m 0755 %{SOURCE5} $RPM_BUILD_ROOT%{_initrddir}/nvidia-newest
...
if [ "$1" -eq "1" ]; then
  ...
  %{_sbindir}/nvidia-newest-config-display enable &>/dev/null ||:
  # Add init script and start it
  /sbin/chkconfig --add nvidia-newest ||:
  /etc/init.d/nvidia-newest start &>/dev/null ||:
fi
...
if [ "$1" -eq "0" ]; then
    ...
    test -f %{_sbindir}/nvidia-newest-config-display && %{_sbindir}/nvidia-newest-config-display disable &>/dev/null ||:
    %{_initrddir}/nvidia-newest stop &> /dev/null ||:
    /sbin/chkconfig --del nvidia-newest ||:
fi ||:
...
* Thu Oct 30 2008 kwizart < kwizart at gmail.com > - 177.80-2
- Rename to nvidia-newest
